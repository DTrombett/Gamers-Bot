...
function getEmoji(flag, client) {
  return client.emojis.cache.find(e => e.name == flag).toString();
}

module.exports = {
  name: 'info',
  description: 'Scopri le informazioni di un utente!',
  async execute(message, args, client, prefix, db, findMember, guild) {
    try {
      ...
      .addFields(
        ...
        { name: 'Data creazione', value: createdAt + ` (\`${passed} ago\`)`},
